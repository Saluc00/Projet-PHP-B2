...
<div class="container">
    <div id="messages" style="height: 60vh;" class="border rounded bourder-dark p-3 m-3 overflow-auto">
        ...
        <div style="text-overflow: ellipsis; word-wrap: break-word;">
            <p><a href="/profile/{{ DB::table('profiles')->where('profile_id', '=', $message->profil_id)->get()[0]->profile_id }}">{{ DB::table('profiles')->where('profile_id', '=', $message->profil_id)->get()[0]->pseudo }}</a>:
                {{ $message->content }}</p>
        </div>
        @endforeach
    </div>
